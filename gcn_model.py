...
class GCN_w_position_embedding(nn.Module):
    ...
    def forward(self, graph_boxes_features, boxes_in_flat):
        ...
        square_threshold = self.cfg.square_threshold

        # My position
        boxes_distance_foot = calc_foot_distance(boxes_in_flat).to(device)
        # boxes_distance_square = calc_spatial_distance(boxes_in_flat).to(device)
        position_mask = (boxes_distance_foot > (pos_threshold * OW))
        # square_mask = (boxes_distance_square > (square_threshold))
        ...
        for i in range(NG):
            ...
            # relation_graph[square_mask] = -float('inf')
...
class G2O(nn.Module):
    ...
    def forward(self, graph_boxes_features):
        ...
        # GCN graph modeling
        # Prepare boxes similarity relation
        B, N, _ = graph_boxes_features.shape
        NFR = self.cfg.num_features_relation
        NG = self.NG
        ...
        for i in range(NG):
            ...
            # Build relation graph
            relation_graph = similarity_relation_graph
            relation_graph = relation_graph.reshape(B, N, N).sum(2)  # 1, N
...
class GCN_temp(nn.Module):
    ...
    def forward(self, graph_boxes_features):
        """
        graph_boxes_features  [N,T,NFG]  process second dim
        """
        # GCN graph modeling
        # Prepare boxes similarity relation
        NFR = self.cfg.num_features_relation
        NG = self.cfg.num_lite_graph
        graph_boxes_features_list = []
        for i in range(NG):
            graph_boxes_features_theta = self.fc_rn_theta_list[i](graph_boxes_features)
            graph_boxes_features_phi = self.fc_rn_phi_list[i](graph_boxes_features)
            similarity_relation_graph = torch.matmul(graph_boxes_features_theta,
                                                    graph_boxes_features_phi.transpose(1, 2))  # N, T, T
            similarity_relation_graph = similarity_relation_graph / np.sqrt(NFR + self.NFP)
            ...
            relation_graph_for_convolution = relation_graph_w_softmax
            one_graph_boxes_features = self.fc_gcn_list[i](
                torch.matmul(relation_graph_for_convolution, graph_boxes_features))  # N, T, NFGONE
            one_graph_boxes_features = self.nl_gcn_list[i](one_graph_boxes_features)
            one_graph_boxes_features = F.relu(one_graph_boxes_features)
            graph_boxes_features_list.append(one_graph_boxes_features)
            
        graph_boxes_features_list = torch.stack(graph_boxes_features_list, dim=0)
        graph_boxes_features_list = torch.sum(graph_boxes_features_list, dim=0)       

        return graph_boxes_features_list

class GCN_spat(nn.Module):
    def __init__(self, cfg):
        super(GCN_spat, self).__init__()
        self.cfg = cfg
        NFR = cfg.num_features_relation
        NG = cfg.num_lite_graph
        NFG = cfg.num_features_gcn
    ...
    def forward(self, graph_boxes_features):
        """
        graph_boxes_features  [T, N, NFG]  process second dim
        """
        # GCN graph modeling
        # Prepare boxes similarity relation
        T, N, NFG = graph_boxes_features.shape  # T, N, NFG
        NFR = self.cfg.num_features_relation
        NG = self.cfg.num_lite_graph
        graph_boxes_features_list = []
        for i in range(NG):
            ...
            graph_boxes_features_phi = self.fc_rn_phi_list[i](graph_boxes_features)
            similarity_relation_graph = torch.matmul(graph_boxes_features_theta,
                                                    graph_boxes_features_phi.transpose(1, 2))  # T,N,N
            similarity_relation_graph = similarity_relation_graph / np.sqrt(NFR + self.NFP)
            ...
            relation_graph_for_convolution = relation_graph_w_softmax
            one_graph_boxes_features = self.fc_gcn_list[i](
                torch.matmul(relation_graph_for_convolution, graph_boxes_features))  # T, N, NFG_ONE
            one_graph_boxes_features = self.nl_gcn_list[i](one_graph_boxes_features)
            one_graph_boxes_features = F.relu(one_graph_boxes_features)
            graph_boxes_features_list.append(one_graph_boxes_features)
