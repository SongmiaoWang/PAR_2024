...
class Config(object):
    ...
    def __init__(self, dataset_name):
        ...
        self.myJson = readJson('/home/yanhaomin/songmiao/PAR/PanoAct_source-code/config.json')
        ...
        self.offline_graph = False
        self.sample_interval = None
        if self.dataset_name == 'JRDB':
            self.data_path = self.myJson["Datasets"][dataset_name]['data_path']
            self.annotation_path = self.myJson["Datasets"][dataset_name]['annotation_path']
            # self.json_anno_path = self.myJson["Datasets"][dataset_name]["json_anno_path"]
            self.train_json_anno_path = self.myJson["Datasets"][dataset_name]["train_json_anno_path"]
            self.test_json_anno_path = self.myJson["Datasets"][dataset_name]["test_json_anno_path"]
            self.test_seqs = [2, 7, 11, 16, 17, 25, 26]
            self.train_seqs = [s for s in range(0, 26) if s not in self.test_seqs]
        elif self.dataset_name == 'PAR':
            self.data_path = self.myJson["Datasets"][dataset_name]['data_path']
            self.train_json_anno_path = self.myJson["Datasets"][dataset_name]["train_json_anno_path"]
            self.test_json_anno_path = self.myJson["Datasets"][dataset_name]["test_json_anno_path"]
            self.vid_id_map_path = self.myJson["Datasets"][dataset_name]["vid_id_map_path"]
            self.test_seqs = [0]
            self.train_seqs = [1]

        # Backbone
        self.backbone = 'inv3'
        self.crop_size = 5, 5  # crop size of roi align 44forMUP 55forOURS
        ...
        self.square_threshold = 4

        # Training Parameters
        self.train_random_seed = 666  # 666
